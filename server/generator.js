...
function buildBlueprint({ prompt, gameType, complexity }) {
  const normalized = String(prompt || "").toLowerCase();
  const safeGameType = String(gameType || "Custom");
  const safeComplexity = String(complexity || "Medium");
...
-- Register remote events, initialize modules, then start game loop.`
  ...
  const title = `${safeGameType} ${safeComplexity} Blueprint`;
  const overview = `This concept is designed as a ${
    complexityMap[safeComplexity] || complexityMap.Medium
  } for a ${safeGameType.toLowerCase()}-style Roblox game. Prompt focus: "${prompt}". The output prioritizes replayability, progression clarity, and monetization hooks that can be layered without breaking early gameplay.`;
